...
const DMSPage = () => {
  ...
  useEffect(() => {
    fetch('/data.json')
      ...
      .then((data: ResourceFile[]) => {
        const dmsData = data.filter(
          (item) =>
            item.metadata.cycle === 'Engineering' &&
            item.metadata.department === 'DMS'
        );

        setFirstYearData(
          dmsData.filter((item) => item.metadata.year === '1st Year')
        );
        setSecondYearData(
          dmsData.filter((item) => item.metadata.year === '2nd Year')
        );
  ...
  const renderSubjects = (data: ResourceFile[], year: string) => {
    ...
    const renderSemester = (semesterData: ResourceFile[], semesterName: string) => {
      const hasContent = semesterData.some(file => file.subjects.length > 0 && file.subjects.some(subject => subject.name.toLowerCase() !== 'coming soon'));

      if (!hasContent && year === '2nd Year') {
        return (
            <div className="mb-8 p-6 bg-gray-100 rounded-lg border border-gray-200 shadow-sm">
                <h4 className="text-xl font-semibold text-gray-500">2nd Year DMS - Coming Soon</h4>
            </div>
        )
      }
        ...
        <section className="mt-8">
          ...
          {semesterData.map((file) =>
            file.subjects.map((subject) => {
              ...
              if (isComingSoon && subject.name.toLowerCase() !== 'dms') {
                return (
              ...
              if(subject.name.toLowerCase() === 'dms') return null;
  ...
  return (
    <div>
      <h1 className="text-4xl font-bold mb-8 border-b pb-4">
        Engineering Cycle - DMS Department
      </h1>

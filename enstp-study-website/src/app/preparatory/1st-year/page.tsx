  ...
  useEffect(() => {
    fetch('/data.json')
      ...
      .then((data: ResourceFile[]) => {
        const firstYearData = data.filter(
          (item) =>
            item.metadata.cycle === 'Preparatory' &&
            item.metadata.year === '1st Year'
        );

        setSemester1Data(
          firstYearData.filter((item) => item.metadata.semester === 'Semester 1')
        );
        setSemester2Data(
          firstYearData.filter((item) => item.metadata.semester === 'Semester 2')
        );
    ...
    return data.map((file) =>
      file.subjects.map((subject) => (
        <div key={subject.name} className="mb-8 p-6 bg-white rounded-lg border border-gray-200 shadow-sm">
          <h3 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">{subject.name}</h3>
